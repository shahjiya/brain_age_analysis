...
filtered_demo_data <- TAY_demographics %>%
...
if (nrow(template_df) <= 1) {
  ...
  template_df <- template_df[0, ]  # Retain column structure but no rows
    ...
    new_data <- filtered_demo_data[, c("clean_subject_id", "Age", "sex")]
    ...
    template_df$clean_subject_id <- filtered_demo_data$clean_subject_id
    template_df$Age <- filtered_demo_data$Age
    template_df$sex <- filtered_demo_data$sex
}
```
...
```{r}
final_template <- left_join(template_df[, c(1:11,16,17)], aseg_data_clean, by = "clean_subject_id") %>%  
...
```{r}
final_template$Age <- as.numeric(final_template$Age)
final_template$GMV <- as.numeric(final_template$GMV)
final_template$WMV <- as.numeric(final_template$WMV)
final_template$sGMV <- as.numeric(final_template$sGMV)
final_template$Ventricles <- as.numeric(final_template$Ventricles)

final_template$age_days <- (final_template$Age * 365.245) + 280
final_template$...1 <- NULL
final_template$INDEX.TYPE <- NULL
final_template$INDEX.OB <- NULL
```
...
output_file <- paste0("../data/processed/populated_brainchart_template.csv")
write.csv(final_template, output_file)
