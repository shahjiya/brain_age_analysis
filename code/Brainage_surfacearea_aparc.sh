...
OUTPUT_FILE="surfacearea_data.csv"
...
for subject in "$SUBJECTS_DIR"/*; do
  if [ -d "$subject" ]; then
    ...
    if [[ -f "$LH_FILE" && -f "$RH_FILE" ]]; then
      ...
      # Extract left hemisphere surfacearea data
      LH_AREA=$(awk '$1 != "#" && NF > 1 {print $3}' "$LH_FILE" | tr '\n' ',' | sed 's/,$//')

      # Extract right hemisphere surfacearea data
      RH_AREA=$(awk '$1 != "#" && NF > 1 {print $3}' "$RH_FILE" | tr '\n' ',' | sed 's/,$//')

      # Combine subject name, thickness data, and hemisphere data
      echo "$SUBJECT_NAME,$LH_AREA,$RH_AREA" >> "$OUTPUT_FILE"
